...
export function useSearchAutoComplete(
    ...
    function findSearchPattern(
        ...
        if (fromMatch) {
            ...
            const fullUsername =
                fromMatch[1].replace("@", "") +
                (continuationMatch ? continuationMatch[1] : "");
    ...
    function onClick(userId: string, username: string) {
        ...
        if (el && state.type === "user") {
            ...
            const clickedIndex = state.matches.findIndex(
                (u) => u._id === userId,
            );
            if (clickedIndex !== -1) {
...
export function transformSearchQuery(
    ...
    const betweenMatch = query.match(
        /\bbetween:(\d{4}-\d{2}-\d{2})\.\.(\d{4}-\d{2}-\d{2})/,
    );
    if (betweenMatch) {
        // Start date: from the START of the first day
        const [startYear, startMonth, startDay] = betweenMatch[1]
            .split("-")
            .map(Number);
        ...
        const [endYear, endMonth, endDay] = betweenMatch[2]
            .split("-")
            .map(Number);
