...
import {
    ...
    UserMapping,
} from "../../lib/hooks/useSearchAutoComplete";
...
export function SearchBar() {
    ...
    const [showDatePicker, setShowDatePicker] = useState<
        "before" | "after" | "during" | "between" | null
    >(null);
    ...
    const [showDuplicateFilterError, setShowDuplicateFilterError] =
        useState(false);
    ...
    const handleFocus = () => {
        ...
        const hasIncompleteFilter = query.match(
            /\b(from:|mentions:|has:|before:|after:|during:|between:)\s*$/,
        );
        if (
            !hasIncompleteFilter &&
            !showDatePicker &&
    ...
    const handleClick = () => {
        ...
        const isInFilter = beforeCursor.match(
            /\b(from:|mentions:|has:|before:|after:|during:|between:)[^\\s]*$/,
        );
    ...
    const handleBlur = () => {
        ...
        setTimeout(() => {
            ...
            const hasIncompleteDateFilter = query.match(
                /\b(before:|after:|during:)\s*$/,
            );
    ...
    const handleInput = (e: Event) => {
        ...
        const incompleteFilterWithSpace = value.match(
            /\b(from:|mentions:)\s+$/,
        );
        ...
        if (beforeMatch) {
            ...
            const filterActive = value.match(
                /\b(before:|after:|during:|between:|has:)\s*$/,
            );
            if (!filterActive) {
                onAutocompleteChange(e);
                if (showDatePicker) {
    ...
    const handleSearch = () => {
        ...
        const hasIncompleteUserFilter = trimmedQuery.match(
            /\b(from:|mentions:)\s*$/,
        );
        const hasIncompleteDateFilter = trimmedQuery.match(
            /\b(before:|after:|during:|between:)\s*$/,
        );
        const hasIncompleteHasFilter = trimmedQuery.match(/\bhas:\s*$/);

        if (
            hasIncompleteUserFilter ||
            hasIncompleteDateFilter ||
            hasIncompleteHasFilter
        ) {
        ...
        if (
            checkDuplicates(
                /\bbetween:\d{4}-\d{2}-\d{2}\.\.\d{4}-\d{2}-\d{2}/gi,
                "between",
            )
        )
            return;
        ...
        const hasFilterMatches =
            trimmedQuery.match(/\bhas:(video|image|link|audio|file)/gi) || [];
        ...
        const dateRangeCount = (trimmedQuery.match(/\bdate-range\b/g) || [])
            .length;
        if (dateRangeCount > 1) {
        ...
        if (dateRangeCount === 1 && activeDateRange) {
            ...
            trimmedQuery = trimmedQuery.replace(
                /\bdate-range\b/,
                `between:${activeDateRange.start}..${activeDateRange.end}`,
            );
        }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if (e.key === "Backspace" || e.key === "Delete") {
            ...
            if (e.key === "Backspace") {
                ...
                const singleDateMatch = beforeCursor.match(
                    /\b(before|after|during):(\d{4}-\d{2}-\d{2})\s*$/,
                );
                if (singleDateMatch) {
                    e.preventDefault();
                    ...
                    const newValue =
                        currentValue.slice(0, filterStart) + afterCursor;
                    setQuery(newValue);
                ...
                const filterOnlyMatch = beforeCursor.match(
                    /\b(before|after|during|between):\s*$/,
                );
                if (filterOnlyMatch) {
                    e.preventDefault();
                ...
                const betweenMatch = beforeCursor.match(
                    /\bbetween:(\d{4}-\d{2}-\d{2})\.\.(\d{4}-\d{2}-\d{2})\s*$/,
                );
                if (betweenMatch) {
                    e.preventDefault();
                    ...
                    const newValue =
                        currentValue.slice(0, filterStart) +
                        "between:" +
                        startDate +
                        ".." +
                        afterCursor;
                    ...
                    const newCursorPos =
                        filterStart + "between:".length + startDate.length + 2;
                    setTimeout(() => {
                        if (inputRef.current) {
                            inputRef.current.setSelectionRange(
                                newCursorPos,
                ...
                const partialBetweenMatch = beforeCursor.match(
                    /\bbetween:(\d{4}-\d{2}-\d{2})\.\.\s*$/,
                );
                if (partialBetweenMatch) {
                    ...
                    const newCursorPos =
                        filterStart + "between:".length + startDate.length;
                    setTimeout(() => {
                        if (inputRef.current) {
                            inputRef.current.setSelectionRange(
                                newCursorPos,
                ...
                const betweenStartOnlyMatch = beforeCursor.match(
                    /\bbetween:(\d{4}-\d{2}-\d{2})\s*$/,
                );
                ...
                const hasMatch = beforeCursor.match(
                    /\bhas:(video|image|link|audio|file)\s*$/,
                );
                if (hasMatch) {
                    e.preventDefault();
                ...
                if (serverWideMatch) {
                    e.preventDefault();
                    const newValue =
                        currentValue.slice(0, serverWideMatch.index) +
                        afterCursor;
                    setQuery(newValue);
                ...
                const filterMatch = afterCursor.match(
                    /^(before|after|during|between):(\d{4}-\d{2}-\d{2})?/,
                );
                ...
                const hasAfter = afterCursor.match(
                    /^has:(video|image|link|audio|file)?\s*/,
                );
                if (hasAfter) {
                    e.preventDefault();
                    const newValue =
                        beforeCursor + afterCursor.slice(hasAfter[0].length);
                    setQuery(newValue);
                    return;
                }
                ...
                if (serverWideAfter) {
                    ...
                    const newValue =
                        beforeCursor +
                        afterCursor.slice(serverWideAfter[0].length);
                    setQuery(newValue);
                    return;
                }
            }
    ...
    const handleOptionClick = (option: SearchOption) => {
        ...
        if (
            ...
            option.label === "after:" ||
            option.label === "during:" ||
            option.label === "between:"
        ) {
            setShowDatePicker(
                option.label.slice(0, -1) as
                    | "before"
                    | "after"
                    | "during"
                    | "between",
    ...
    useEffect(() => {
        if (showDatePicker || showAttachmentTypes) {
            const handleClickOutside = (e: MouseEvent) => {
                ...
                if (
                    !container.closest("[data-search-container]") &&
                    !container.closest("[data-date-picker]")
                ) {
                    setShowDatePicker(null);
                    setShowAttachmentTypes(false);
                }
