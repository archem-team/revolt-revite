...
function buildPayload(form: VendorData, isReseller: boolean) {
    return {
        ...
        guarantee: {
            ...
            purityPct: form.purityPct ? Number(form.purityPct) : null,
            volume: form.guarantees.volume,
            volumePct: form.volumePct ? Number(form.volumePct) : null,
            reship: form.guarantees.reship,
            reshipDesc: form.reshipText,
        },
...
const S = {
    ...
    btn: (variant: "primary" | "outline") =>
        ({
            ...
            border:
                variant === "primary"
                    ? "none"
                    : "1px solid var(--tertiary-foreground)",
        } as const),
    status: (ok: boolean) =>
        ({
            fontSize: "13px",
    ...
    divider: {
        borderTop: "1px solid var(--tertiary-foreground)",
        margin: "20px 0",
    } as const,
};
...
export const VendorInfo = observer(({ server }: Props) => {
    ...
    async function handleSave(e: Event) {
        ...
        try {
            const r = await fetch(
                `${API_BASE}/directory/communities/owner/${server._id}`,
                {
                    method: "PATCH",
                    headers: {
    ...
    if (loadState === "loading")
        return <div style={S.wrap}>Loading vendor info…</div>;
        ...
        return (
            <div style={S.wrap}>
                ...
                <p
                    ...
                    This server does not have a directory listing. Contact an
                    admin to get listed.
                </p>
            </div>
        );
        ...
        <div style={S.wrap}>
            ...
            <form onSubmit={handleSave}>
                ...
                <div style={S.section}>
                    ...
                    <div style={S.grid}>
                        {(
                            Object.keys(
                                PAYMENT_LABELS,
                            ) as (keyof typeof PAYMENT_LABELS)[]
                        ).map((k) => (
                            <label
                                key={k}
                                style={S.checkItem(!!form.payment[k])}>
                                <input
                                    type="checkbox"
                                    style={{ display: "none" }}
                ...
                <div style={S.section}>
                    ...
                    <div style={S.grid}>
                        {(
                            ...
                            <label
                                key={k}
                                style={S.checkItem(!!form.warehouses[k])}>
                                <input
                                    type="checkbox"
                                    style={{ display: "none" }}
                                    checked={!!form.warehouses[k]}
                                    onChange={() =>
                                        setForm((f) => ({
                                            ...f,
                                            warehouses: toggle(f.warehouses, k),
                                        }))
                                    }
                                />
                                {WAREHOUSE_LABELS[k]}
                ...
                <div style={S.section}>
                    ...
                    <div style={S.grid}>
                        {(
                            ...
                            <label
                                key={k}
                                style={S.checkItem(!!form.products[k])}>
                                <input
                                    type="checkbox"
                                    style={{ display: "none" }}
                                    checked={!!form.products[k]}
                                    onChange={() =>
                                        setForm((f) => ({
                                            ...f,
                                            products: toggle(f.products, k),
                                        }))
                                    }
                                />
                                {PRODUCT_LABELS[k]}
                        ...
                        <div key={k} style={{ marginBottom: "12px" }}>
                            <label style={S.checkItem(form.guarantees[k])}>
                                <input
                                    type="checkbox"
                                    style={{ display: "none" }}
                                    checked={form.guarantees[k]}
                                    onChange={() =>
                                        setForm((f) => ({
                                            ...f,
                                            guarantees: toggle(f.guarantees, k),
                                        }))
                                    }
                                />
                                {GUARANTEE_LABELS[k]}
                            ...
                            <input
                                ...
                                placeholder={
                                    numeric
                                        ? "% e.g. 99"
                                        : "e.g. reship if seized by customs"
                                }
                                style={{
                                    ...S.input,
                                    marginTop: "4px",
                                ...
                                onInput={(e) =>
                                    setForm((f) => ({
                                        ...f,
                                        [field]: (e.target as HTMLInputElement)
                                            .value,
                                    }))
                                }
                            />
                ...
                <div style={S.section}>
                    ...
                    <div style={S.row}>
                        <input
                            type="text"
                            style={S.input}
                            placeholder="Shipping time e.g. 3-5 days"
                            value={form.shippingTime}
                            onInput={(e) =>
                                setForm((f) => ({
                                    ...f,
                                    shippingTime: (e.target as HTMLInputElement)
                                        .value,
                                }))
                            }
                        />
                        <input
                            ...
                            min={0}
                            style={S.input}
                            placeholder="Free shipping threshold e.g. 150"
                        ...
                        <div style={S.section}>
                            ...
                            <div style={S.grid}>
                                {(
                                    ...
                                    <label
                                        key={k}
                                        style={S.checkItem(form.orderTypes[k])}>
                                        <input
                                            type="checkbox"
                                            style={{ display: "none" }}
                                            checked={form.orderTypes[k]}
                                            onChange={() =>
                                                setForm((f) => ({
                                                    ...f,
                                                    orderTypes: toggle(
                ...
                <div
                    ...
                    {saveState === "ok" && (
                        <span style={S.status(true)}>Saved!</span>
                    )}
                    {saveState === "error" && (
                        <span style={S.status(false)}>{errMsg}</span>
                    )}
                </div>
            </form>
        </div>
