...
const copy = {
    en: {
        ...
        activeBody: "Vendors who continue supporting PepChat are more likely to stay visible in the ranking. Older contributions do not disappear after 30 days. Their value decreases gradually, so every contribution still counts while recent support has a stronger effect.",
        summaryTitle: "The short version",
        summary: "On the day we record a contribution, $1 adds 1 point. After 60 days, it is worth half as much. After 120 days, it is worth one quarter. We calculate the score from each contribution, not from the lifetime total.",
        ...
        eliteRule: "Active Elite placements come first. The open spots go to servers with the highest positive scores.",
        pro: "Pro",
        proPos: "Positions 4-9",
        proRule: "Active Pro placements come first. The open spots go to the servers with the next highest positive scores.",
        green: "Green Zone",
        greenPos: "After position 9",
        greenRule: "A current score of 100 or more. Servers are ordered by score.",
        fresh: "New",
        freshPos: "After Green",
        freshRule: "Servers marked New with less than $100 in lifetime support.",
        standard: "Standard",
        standardPos: "Remaining positions",
        standardRule: "Everyone else, ordered by score.",
        overrideNote: "Some agreements reserve an Elite, Pro, or Green placement for a set period. They can also change the server's colour. These placements do not add points to the score.",
        exampleTitle: "A worked example",
        exampleIntro: "We calculate every contribution separately. Here is what three $1,000 contributions would be worth on the same day:",
        today: "today",
        days60: "60 days ago",
        ...
        calcIntro: "Use this to estimate your score and check whether it reaches the Green Zone. It cannot predict an exact position because other server scores and active placements also affect the order.",
        ...
        tierCaveat: "A top-nine score may place a server in Elite or Pro. Your exact position depends on the live ranking.",
        detailsTitle: "Details that matter",
        detail1Title: "Scores age daily.",
        detail1: "A contribution does not expire on a fixed date. Its value halves every 60 days and keeps decreasing.",
        detail2Title: "The 100-point rule is current score, not lifetime total.",
        detail2: "A server may have contributed more than $100 over time but sit below Green after older contributions decay.",
        detail3Title: "Equal scores have deterministic tie-breakers.",
        detail3: "If two scores match, we compare lifetime support and then the server ID. The order is not random.",
        detail4Title: "Updates are automatic.",
        detail4: "We request a new calculation when a contribution is recorded. A scheduled job also refreshes the ranking, so a short delay is possible.",
    ...
    zh: {
        ...
        back: "返回 PepChat",
        activeTitle: "持续支持很重要",
        activeBody: "持续支持 PepChat 的商家更有机会在排名中保持较高的曝光。旧捐赠不会在 30 天后突然清零，而是逐渐降低分值。因此，每笔支持都会继续计入得分，近期支持的影响更大。",
        summaryTitle: "简要说明",
        summary: "捐赠记录当天，1 美元等于 1 分；60 天后剩一半，120 天后剩四分之一。系统根据每笔捐赠重新计算，而不是直接使用历史累计金额。",
        formulaTitle: "计算公式",
        ...
        overrideNote: "位置协议具有开始和到期日期，可保留 Elite、Pro 或 Green 位置，也可能改变标识颜色。协议不会永久增加得分。",
        exampleTitle: "计算示例",
        exampleIntro: "三笔捐赠会分别计算，不会合并成一个历史余额。在同一个计算日期：",
        today: "今天",
        days60: "60 天前",
        days120: "120 天前",
        points: "分",
        ...
        calcIntro: "此计算器仅显示分值和 Green Zone 资格，不承诺具体位置；其他服务器的得分和有效位置协议都会影响排名。",
        currentLabel: "当前得分",
        ...
        days: "天",
        estimatedScore: "预计得分",
        tierCaveat: "如果得分进入前九名，可能获得 Elite 或 Pro。具体位置只能通过实时排名确定。",
        detailsTitle: "需要注意的细节",
        detail1Title: "得分会逐日衰减。",
        detail1: "捐赠不会突然失效；其权重每 60 天减半，并继续逐渐降低。",
        detail2Title: "100 分指当前得分，不是历史累计金额。",
        detail2: "服务器历史累计可能超过 100 美元，但旧捐赠衰减后，当前得分仍可能低于 Green。",
        detail3Title: "同分时采用固定规则。",
        detail3: "系统先比较历史累计支持，再比较服务器 ID。Green 区域不会随机洗牌。",
        detail4Title: "系统会自动更新。",
        detail4: "记录捐赠后会请求重新计算，也可以通过定时任务运行。处理过程可能存在短暂延迟。",
        faqTitle: "常见问题",
...
export default function Ranking() {
    ...
    useEffect(() => {
        ...
        document.title = language === "en" ? "How Ranking Works · PepChat" : "排名机制说明 · PepChat";
        return () => {
    ...
    return (
        <div className="ranking-page" lang={language === "zh" ? "zh-CN" : "en"}>
            <header className="ranking-nav">
                <a className="ranking-wordmark" href="/" aria-label="PepChat home">
                    PepChat
                </a>
                <div className="ranking-nav-actions">
                    ...
                    <a className="ranking-back" href="/">{t.back}</a>
                </div>
            </header>
            ...
            <main className="ranking-shell">
                <article className="ranking-document">
                    ...
                    <aside className="ranking-active-support">
                        <span className="ranking-active-mark" aria-hidden="true" />
                        <div>
                            <h2>{t.activeTitle}</h2>
                            <p>{t.activeBody}</p>
                        </div>
                    ...
                    <section>
                        ...
                        <p>{t.summary}</p>
                        <div className="ranking-formula" aria-label={t.formulaTitle}>
                    ...
                    <section>
                        ...
                        <div className="ranking-table-wrap">
                            <table className="ranking-tier-table">
                                <thead><tr><th>{t.highlight}</th><th>{t.positions}</th><th>{t.qualification}</th></tr></thead>
                                <tbody>
                                    {tierRows.map(([kind, label, position, rule]) => (
                                        <tr key={kind}>
                                            <td data-label={t.highlight}><span className={`ranking-tier ranking-tier--${kind}`}>{label}</span></td>
                                            <td data-label={t.positions}>{position}</td>
                                            <td data-label={t.qualification}>{rule}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <section>
                        ...
                        <div className="ranking-example">
                            <div><span>$1,000 · {t.today}</span><strong>1,000 {t.points}</strong></div>
                            <div><span>$1,000 · {t.days60}</span><strong>500 {t.points}</strong></div>
                            <div><span>$1,000 · {t.days120}</span><strong>250 {t.points}</strong></div>
                            <div className="ranking-example-total"><span>{t.total}</span><strong>1,750 {t.points}</strong></div>
                        </div>
                    </section>
                    ...
                    <section className="ranking-calculator">
                        ...
                        <div className="ranking-fields">
                            <label>
                                ...
                                <input type="number" min="0" step="0.01" value={currentScore} onInput={(event) => setCurrentScore(event.currentTarget.value)} />
                                <small>{t.currentHint}</small>
                            </label>
                            <label>
                                <span>{t.supportLabel}</span>
                                <input type="number" min="0" step="1" value={newSupport} onInput={(event) => setNewSupport(event.currentTarget.value)} />
                        ...
                        <div className="ranking-result" aria-live="polite">
                            ...
                            <p className={projected >= 100 ? "is-qualified" : "is-below"}>
                                <span aria-hidden="true">{projected >= 100 ? "✓" : "○"}</span>{" "}
                                {projected >= 100 ? t.qualifies : t.notQualifies}
                            </p>
                            {gap > 0 && <p>{t.gap}: <strong>{gap.toFixed(2)}</strong></p>}
                        </div>
                        <div className="ranking-projection">
                            {futureDays.map((days) => (
                                <div key={days}>
                                    <span>{t.after} {days} {t.days}</span>
                                    <strong>{(projected * Math.pow(2, -days / 60)).toFixed(2)}</strong>
                    ...
                    <section>
                        ...
                        <div className="ranking-details">
                            {[[t.detail1Title, t.detail1], [t.detail2Title, t.detail2], [t.detail3Title, t.detail3], [t.detail4Title, t.detail4]].map(([title, body]) => (
            ...
            <footer className="ranking-footer"><span>{t.footer}</span><a href="/">{t.back}</a></footer>
        </div>
    );
}
