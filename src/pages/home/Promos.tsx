...
export function getComparableCompound(promo: Promo): string | null {
    const rawKey = (promo as any).productKey || (promo.items && promo.items.length > 0 ? promo.items[0].product : null);
    if (!rawKey) return null;
    const lower = rawKey.toLowerCase();
    if (lower.includes("reta")) return "retatrutide";
    if (lower.includes("tirz")) return "tirzepatide";
    if (lower.includes("sema")) return "semaglutide";
    if (lower.includes("hgh") || lower.includes("growth") || lower.includes("somatropin")) return "hgh";
    return null;
}
...
function getSearchScore(p: Promo, q: string): number {
    ...
    if (query.includes("reta")) query = "retatrutide";
    else if (query.includes("tirz")) query = "tirzepatide";
    else if (query.includes("sema")) query = "semaglutide";
    else if (query.includes("hgh") || query.includes("growth")) query = "hgh";
...
const Grid = styled.div`
    ...
    max-width: 100%;
    box-sizing: border-box;
    /* Prevent column-count layout from creating horizontal overflow on mobile */
    overflow-x: hidden;
...
const Gallery = styled.div`
    ...
    flex-direction: column;
    gap: 8px;
    /* Prevent images from overflowing card bounds on mobile */
    overflow: hidden;
    min-width: 0;
    max-width: 100%;
    ...
    .thumbs {
        ...
        gap: 6px;
        overflow-x: auto;
        /* Prevent thumbnail row from creating vertical overflow */
        overflow-y: hidden;
        max-width: 100%;
...
function ComparisonDrawer({
    ...
    useEffect(() => {
        ...
        // Also lock <html> scroll — required for iOS Safari to prevent body scroll
    ...
    const minPriceVal = vendors.length > 0 ? Math.min(...vendors.map((v) => v.minPrice)) : 0;
    const nonLowestDiscounted = vendors
        .filter((v) => v.minPrice > minPriceVal && (v.discount || v.hasActivePromo !== false))
        .sort((a, b) => a.minPrice - b.minPrice);

    function getRankLabel(v: typeof vendors[0]): string | null {
        if (v.minPrice === minPriceVal) return "🥇 Lowest Price";
    ...
    return (
        <>
            ...
            <CompareDrawerContainer>
                ...
                {/* No Active Promotions Notice Banner */}
                                    ...
                                    {isExpanded && (
                                        <div className="vendor-card-body">
                                            ...
                                            <div className="specs-grid">
                                                ...
                                                {v.discount && v.discount !== "No Active Promo" && (
                                                    <div className="spec-item">
                                                        <span>Discount</span>
                                                ...
                                                <div className="spec-item">
                                                    ...
                                                    <strong style={{ color: hasPromo ? "var(--status-online, #10b981)" : "var(--tertiary-foreground)" }}>
                                                        {hasPromo ? "Ready to Ship" : "Out of Stock"}
                                                    </strong>
                                                </div>
                                            </div>
                ...
                return (
                    <ComparisonDrawer
                        ...
                        onApplyFilter={(productKey) => {
                            ...
                            let matchedKey: FilterKey = "all";
                            if (lower.includes("reta")) matchedKey = "retatrutide";
                            else if (lower.includes("tirz")) matchedKey = "tirzepatide";
                            else if (lower.includes("sema")) matchedKey = "semaglutide";
                            else if (lower.includes("hgh") || lower.includes("growth")) matchedKey = "hgh";

                            setActiveFilter(matchedKey);
                            onClose();
